...
test("an unclosed YAML quote cannot hide a later publish", () => {
  for (const label of ["single", "double"]) {
    const quoted = label === "single" ? "package's release" : '"package release';
    const result = auditPublishAttestation([{
      file: "release.yml",
      text: [
    ...
    assert.equal(result.failures.length, 1, `${label} quote must not hide the later publish`);
    assert.match(result.failures[0]!, /npm publish --access public --ignore-scripts/);
  }
});

test("array flags come only from a live declaration before the invocation", () => {
  const cases = [
    [
      "npm publish --provenance",
  ...
  for (const lines of cases) {
    ...
    assert.equal(result.failures.length, 1, lines.join(" | "));
  }
});
